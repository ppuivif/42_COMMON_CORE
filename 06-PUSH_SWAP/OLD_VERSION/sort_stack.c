...
/*void	ft_sort_50(t_list **src, t_list **dest)
{
	...
	median_src = size_src / 2;
	hits = 0;//to delete
	i = 1;
	j = 1;
	
	while (i + median_src / j <= size_src)
	{
		sort_step_ab(src, j, i);
		i++;
	}
	ft_lst_dc_print((*src)->head);
	i = 1;
	while ((*src)->head)
	{
		hits += search_best_sort_next_position_ab(src, dest, i);
		i++;
	}
...
//	ft_lst_dc_print((*dest)->head);
	...
	i = size_src;
	j = 2;
	while (i - median_src / j > 0)
	{
		while (i > (j + 1) * median_src / j)
		{
			sort_step_ba_1(dest, j, i);
			i--;
		}
		i = median_src;	
		while (i - median_src / j > 0)
		{
			sort_step_ba_2(dest, j, i);
			i--;
		}
		i--;
	}
	...
	while ((*dest)->head)
	{
		hits += search_best_sort_next_position_ba(src, dest, i);
		i++;
	}
