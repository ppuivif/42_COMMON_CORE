...
int RPN::calculate(char character){
	...
	if (_stack.size() < 2){
		std::cerr << BOLD_RED << "Error : calculation is not possible (numbers are missing)" << NORMAL << std::endl;
		return (1);
	}
	...
	if (character == '/'){
		if (secondOperand != 0)	
			...
			std::cerr << BOLD_RED << "Error : calculation is not possible (division per zero)" << NORMAL << std::endl;
			return (1);
		}
	}
...
int RPN::loadCharacters(char *argv){
	...
	std::string remainingString = argv;
	
	while (remainingString.size()){
		std::string character = getNextCharacter(remainingString);
		if (character.size() > 1){
			std::cerr << BOLD_RED << "Error : at least one argument is invalid" << NORMAL << std::endl;
			return (1);
		}
		if (character.size() == 1){
...
void RPN::displayResult(){
	...
	if (this->_stack.size() == 1)
		...
		std::cerr << BOLD_RED << "Error : calculation is not possible (too many numbers)" << NORMAL << std::endl;
}
