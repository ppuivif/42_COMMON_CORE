...
void create_tiles(t_list_image *t_list)//to create images
{
	int width;
	int height;
...
void create_sprites(t_list_image *t_list)//to create images
{
	int width;
	int height;
	...
	t_list->img_p5 = mlx_png_file_to_image(t_list->t_win1->mlx, "Images/imgP5.png", &width, &height);
	if (t_list->img_p == NULL || t_list->img_p1 == NULL || t_list->img_p2 == NULL ||
		t_list->img_p3 == NULL || t_list->img_p4 == NULL || t_list->img_p5 == NULL)
	{
		printf("Error\nAn image didn't could be created\n");
		exit (1);
	}
